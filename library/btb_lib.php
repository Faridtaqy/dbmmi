...
class btb_lib extends db_utility_lib
{
	...
	* Provinsi
	*/
	public function base_btb_provinsi(){
		return array(
			'1' => 'DKI Jakarta',
			'2' => 'Aceh',
			'3' => 'Sumatra Utara',
			'4' => 'Sumatra Barat',
			'provinsi_riau' => 'Riau',
			'provinsi_kepri' => 'Kepulauan Riau',
			'provinsi_jambi' => 'Jambi',
			'provinsi_sumsel' => 'Sumatra Selatan',
			'provinsi_bengkulu' => 'Bengkulu',
			'provinsi_babel' => 'Bangka Belitung',
			'' => 'Lampung',
			'' => 'Jawa Barat',
			'' => 'Jawa Tengah',
			'' => 'Banten',
	...
	* Provinsi
	*/
	public function base_btb_inner_join_provinsi(){
		$status = 500; $message = 'Tidak ada aksi yang dilakukan.';

		$status = (isset($_SESSION['provinsi']) AND trim($_SESSION['provinsi'])!='')?$_SESSION['provinsi']:((isset($response['provinsi']) AND trim($response['provinsi'])!='')?$response['provinsi']:'');
	$message = (isset($_SESSION['message']) AND trim($_SESSION['message'])!='')?base64_decode($_SESSION['message']):((isset($response['message']) AND trim($response['message'])!='')?$response['message']:'');
		
		$sql = '
		SELECT *
		FROM mmi_btb 
		INNER JOIN mmi_provinsi ON mmi_provinsi.id_provinsi = mmi_btb.btb_provinsi
		WHERE id_provinsi='.intval($id_provinsi).'"';

		$exec = $db_utility_lib->query($sql);
		$mmi_provinsi_arr = $exec->row_arr();
			if (!empty($mmi_provinsi_arr)){
	...
	* Regional
	*/
	public function base_btb_regional(){
		return array(
			'ACH' => 'Aceh',
			'MDN' => 'Sumatra Utara',
		);
