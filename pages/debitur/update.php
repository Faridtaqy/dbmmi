<?php if ( ! defined('FCPATH')) exit('No direct script access allowed');
	...
	if (isset($_POST['save']) AND trim(strtolower($_POST['save']))=="simpan") {
		...
		$response = $debitur_lib->form_validation($id);
		if ($response['status']==200) {
			$response = $debitur_lib->stored_data($id);
			if ($response['status']==200) {
	...
	// dapatkan data debitur
	if(isset($_GET['id']) AND is_numeric($_GET['id'])){
		$detail_arr = $debitur_lib->get_detail_debitur($_GET['id']);
		if (empty($detail_arr)) {
			header('Location: '.$websiteConfig->base_url().'?route=debitur');
		}
	} else{
		header('Location: '.$websiteConfig->base_url().'?route=debitur');
	}
...
<div class="row">
	<div class="col-md-12">
		<h3 class="page-header">Ubah Data</h3>
	</div>
</div>
		...
		<div class="panel">
			<div class="panel-body">
				<form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
					...
					<div class="form-group">
						<label class="col-sm-3 control-label">Nama Atasan Penilai</label>
						<div class="col-sm-8">
							<input class="form-control"	 type="text" name="client_atasan_penilai" value="<?php echo (isset($_POST['client_atasan_penilai']) AND trim($_POST['client_atasan_penilai'])!='')?$_POST['client_atasan_penilai']:((isset($detail_arr['client_atasan_penilai']) AND trim($detail_arr['client_atasan_penilai'])!='')?$detail_arr['client_atasan_penilai']:''); ?>">
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="file" name="debitur_photo">
						</div>
					</div>
					<div class="form-group">
