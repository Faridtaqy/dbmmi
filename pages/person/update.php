<?php if ( ! defined('FCPATH')) exit('No direct script access allowed');
	...
	if (isset($_POST['save']) AND trim(strtolower($_POST['save']))=="simpan") {
		$response = $person_lib->form_validation($_GET['id']);
		if ($response['status']==200) {
			$response = $person_lib->stored_data($_GET['id']);
			if ($response['status']==200) {
				header('Location: '.$websiteConfig->base_url().'?route=person&id='.$_GET['id'].'&status='.$response['status'].'&message='.base64_encode($response['message']));
			}
		}
	}
	...
	if(isset($_GET['id']) AND is_numeric($_GET['id'])){
		$detail_arr = $person_lib->get_detail_person($_GET['id']);
		if (empty($detail_arr)) {
			header('Location: '.$websiteConfig->base_url().'?route=person');
		}
	} else{
		header('Location: '.$websiteConfig->base_url().'?route=person');
	}
