<?php if ( ! defined('FCPATH')) exit('No direct script access allowed');
	...
	if (isset($_POST['save']) AND trim(strtolower($_POST['save']))=="simpan") {
		$response = $person_lib->form_validation();
		if ($response['status']==200) {
			$response = $person_lib->stored_data();

			header('Location: '.$websiteConfig->base_url().'?route=person&status='.$response['status'].'&message='.base64_encode($response['message']));
		}
	}
